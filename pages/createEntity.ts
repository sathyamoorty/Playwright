  ...
  await this.page.getByRole('treeitem', { name: 'Assigned To' }).click();
    ...
    async sourceModule(){
         await this.page.locator('#select2-Currentvalue1-container').click();
          ...
          await this.page.getByLabel('Leads').getByRole('treeitem', { name: 'Name' }).click();
    }
    async addFieldBtn(){
    ...
    async mapTheFields(){
        ...
        await this.page.getByRole('treeitem',{name:"Related Module Test"}).click();
    }
    async mapTheField2(){
        await this.page.locator("#select2-Currentvalue2-container").click();
        await this.page.getByRole('treeitem',{name:"Lead Number"}).click();
    }
    async popupBtn(){
    ...
    async dataForLeads(name:string, companyName:string, currency:string){
        await this.page.locator('input[name="leads_companyname"]').fill(companyName);
        await this.page.locator('input[name="leads_name"]').fill(name);
    ...
    async relatedModule(){
        await this.page.locator("#rsoft-All_ui_module").click();
